describe 'Expectation Matchers' do
  ...
  describe 'equivalence matchers' do
    ...
    it 'will match value equality with #eql' do
      ...
      c = 17
      d = 17.0
      expect(c).not_to eql(d)  # not the same, close  
    end
    ...
    it 'will match identity equality with #equal' do
      ...
      c = b
      expect(b).to equal(c)   # same object now
  ...
  describe 'predicate matchers' do
    ...
    it 'will match have_* to custom methods like has_*?' do
      ...
      expect(customer).not_to have_hair # my own edition
    end  
  end

  describe 'observation matchers' do
    # Note that all of these user "expect {}", not "expect()".
    # It is a special block format that allows a 
    # process to take place inside of the expection
    ...
    it 'will match when errors are raised' do
      # observers any errors raised by the block
  ...
  describe 'composing matchers' do
    ...
    it 'will match using noun-phrase aliases for matchers' do
      # These are built-in aliases that make specs read better by
      # using noun-based phrases instead of verb-based phrases
