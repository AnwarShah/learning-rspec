...
describe Guide do
  ...
  describe '#intialize' do
    ...
    it 'calls Restaurant#load_file with its path argument' do
      # stub to allow :load_file
      allow(Restaurant).to receive(:load_file).with(test_file)
      
      Guide.new(test_file)
      # expect that it is received
  ...
  describe '#launch!' do
    ...
    it 'outputs a introductory message' do
      ...
      expect(output).to match(/<<< Welcome to the Food Finder >>>/)
    end
  ...
  describe 'performing actions' do
    ...
    context 'with invalid action' do
      ...
      it 'outputs list of valid actions' do
        ...
        # get the output
        output = capture_output { subject.launch! }
        # get the output into line by one
        lines = output.split("\n")

        # expect line[8] to be list of actions
        # NOTE: previous input will be present
        expect(lines[8]).to match(/Actions: list, find, add, quit/)
      end
    ...
    context 'with quit action' do
      ...
      it 'outputs concluding message and exits' do
        ...
        lines = output.split('\n')
        # lines[0] will be quit message
        expect(lines[0]).to match(/<<< Goodbye and Bon Appetit! >>>/)
      end
    ...
    context 'with list action' do
      ...
      it 'sorts alphabetically with an invalid sort by' do
        ...
        # Build array with the first characters
        first_chars = names.map {|l| l[0] }
        # expect they equal to equivalent sorted array
    ...
    context 'with find action' do
      ...
      it 'outputs instructions if no arguments given' do
        ...
        output = capture_output { subject.launch! }
        expect(output).to match(/Find using a key phrase to search the restaurant list./)
      end
      ...
      it 'finds restaurants with matching name keyword' do
        ...
        lines = output.split("\n")
        expect(lines[11]).to eq("-" * 60)
        expect(lines[12]).to eq(" Cafe Masala                    Indian               $30.00")
        expect(lines[13]).to eq("-" * 60)
      end
      ...
      it 'finds restaurants with matching cuisine keyword' do
        ...
        lines = output.split("\n")
        expect(lines[11]).to eq("-" * 60)
        expect(lines[12]).to eq(" Hot Tamale                     Mexican              $25.00")
        expect(lines[13]).to eq("-" * 60)
      end
      ...
      it 'finds restaurants with prices less than keyword' do
        ...
        lines = output.split("\n")
        expect(lines[11]).to eq("-" * 60)
        expect(lines[12]).to eq(" Pita Pocket                    Fast Food            $10.00")
        expect(lines[13]).to eq(" Quick Cup                      Coffee                $5.00")
        expect(lines[14]).to eq(" Taste Of Little Italy          Pizza                $10.00")
    ...
    context 'with add action' do
      ...
      it 'sends question answers to Restaurant.new' do
        ...
        expect(Restaurant).to have_received(:new).with(
          name: 'Chelsea Diner', cuisine: 'American', price: '20')
      end
